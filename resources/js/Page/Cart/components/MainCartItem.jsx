...
const CartItem = ({cartItem}) => {
	...
	const handleRemoveItem = (e) => {
		dispatch(removerCartItem(cartItem))
	}

	const handleIncrementQty = () =>{
		dispatch(increseQuantity(cartItem))
	}
	...
	return (
		<div key={cartItem.id} className="p-4 my-4 sm:h-[10rem] h-fit rounded-lg overflow-hidden flex flex-col sm:flex-row relative items-center justify-center border-slate-400 border-[0.05rem]">

			<div className="md:w-[6rem] md:h-[8rem] h-[20rem]  flex items-center justify-center me-4 bg-slate-800">
				<img 
			...
			<div className="flex w-full">
				...
				<div className="col-span-1 flex-1 flex flex-col justify-center items-center">
					<div className="flex">
						<button 
							className="bg-slate-800 text-white w-8 h-8"
							onClick={handleIncrementQty}
						>
						+
						</button>
						<div className="w-12 text-center flex items-center justify-center">{cartItem.quantity}</div>
						<button 
							className="bg-slate-800 text-white w-8 h-8"
